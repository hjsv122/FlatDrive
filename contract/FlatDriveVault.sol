...
contract FlatDriveVault {
    ...
    uint256 public minWithdraw = 50 * 1e6; // 50 USDT

    event Credited(uint256 totalUsd, uint256 toPlayer, uint256 toFund);
    event Withdrawn(address indexed to, uint256 gross, uint256 fee, uint256 net);
    ...
    modifier onlyPlayer(){ require(msg.sender == player, "only player"); }
    ...
    function credit(uint256 totalUsdAmount) external onlyOwner {
        ...
        uint256 toFund   = (totalUsdAmount * 10) / 100;
        uint256 toPlayer =  totalUsdAmount - toFund;
        fundBalance   += toFund;
        playerBalance += toPlayer;
        emit Credited(totalUsdAmount, toPlayer, toFund);
    }
    ...
    function withdraw(uint256 amount) external onlyPlayer {
        ...
        require(token.transfer(player, net), "token transfer failed");
        emit Withdrawn(player, amount, fee, net);
    }
